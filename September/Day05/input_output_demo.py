...
class MYagentOutputSchema(TypedDict):

    llm__answer: str

#1.3定义一个InputSchema
class MYagentInputSchema(TypedDict):
    query: str
...
#3构建builder
#传入input_schema和output_schema过滤输入与输出
bulider = StateGraph(state_schema=MYagentState,input_schema=MYagentInputSchema,output_schema=MYagentOutputSchema)
